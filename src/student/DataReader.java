...
public class DataReader {
    ...
    public static ItemInterface read(String path) throws Exception {
        try (Scanner scanner = scannerFor(path)) {
            ...
            if (firstLine.contains("book")) {          // DO NOT MODIFY
            ...
            } else if (firstLine.contains("other")) {
                ...
                return new Order(name, description, price, false);
            }
    ...
    private static Order readBook(String title, String author, int edition, int nPages, String desc, double price) {
        String description = "Author: " + author + " | Edition: " + edition;
        description += " | Number of pages: " + nPages;
        description += " | Product description: " + desc;
        return new Order(title, description, price, true);
    }
}